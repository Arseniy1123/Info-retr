...
# idf
idf_value = np.log10(len(ideal_collection) / df_value) # idf
...
tf_value = [[int(x) if x == 0 else x for x in row] for row in tf] # tf
...
tf_query_value = [[int(x) if x == 0 else x for x in row] for row in tf_query]
...
query_tf_idf = norm_query[num_of_query]

bm25_scores = []
document_indices = list(range(len(collection)))

total_doc_length = sum(len(doc) for doc in collection)
average_doc_length = total_doc_length / len(collection)
tokens = queries_list[num_of_query]
print(tokens)
...
for i, doc in enumerate(collection):
    ...
    for token in tokens:
        if token in doc:
            token_index = doc.index(token)
            idx = w.index(token) # Индекс слова из запроса
            tfidf = query_tf_idf[idx] # Индекс текущего токена в текущем документе
            bm = (tfidf * (k1 + 1)) / (tf_value[i][token_index] / doc_len + k1 * ((1 - b) + b * (doc_len / average_doc_length)))
            bm25_score += bm
            if token == 'мыс' or token == 'спартивенто':
                print(token, bm)
    bm25_scores.append(bm25_score)

sorted_document_indices = np.argsort(bm25_scores)[::-1]

qw = 0
for idx in sorted_document_indices:
    print(f"Вес: {bm25_scores[idx]}, Предложение: {sentences[idx]}\n")
    qw += 1
    if qw == 5:
        break
...
with open("bm_2.txt", "w", encoding="utf-8") as file:
    line = f"Запрос: {queries[num_of_query]}\n"
    file.write(line)

    for idx in sorted_document_indices:
        line = f"Вес: {bm25_scores[idx]}, Предложение: {sentences[idx]}\n"
        file.write(line)


with open("bm_2.txt", "r", encoding="utf-8") as input_file:
    with open("result_bm_2.txt", "w", encoding="utf-8") as output_file:
